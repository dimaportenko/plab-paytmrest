...
class PaytmResponse implements \Plab\PaytmRest\Api\PaytmResponseInterface
{
...
//        $returnUrl = $this->getPaytmHelper()->getUrl('/');
        ...
        if(trim($paytmOrderId)!='' && trim($orderMID)!=''){
            ...
            $resCode = $responseData['RESPCODE'];
            $orderTxnAmount = $responseData['TXNAMOUNT'];
            $transactionResponse="";
            $paytmJsonResponseOnPending='';
            // TODO: replace request
            if($this->getPaytmModel()->validateResponse($responseData, $magentoOrderId)) {
